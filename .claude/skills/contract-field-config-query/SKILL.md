...
从用户问题中提取或确认以下 5 个维度：
1. **业态** (business_type)：1=整装, 2=团装, 3=局装, 4=翻新全案
2. **品牌**：被窝 or 圣都
3. **合同类型** (contract_type)：见 ContractTypeEnum
4. **版本** (version)：根据品牌确定
...
1. **维度完整性**：必须提供全部 5 个维度，缺少任何一个都无法查询
2. **品牌区分**：被窝和圣都的 city_code、company_code、version 取值完全不同
3. **版本含义**：version 在被窝和圣都场景下的含义不同（被窝只有 1 个版本，圣都有 3 个版本）
4. **兜底配置**：圣都的 gb_code=0 和 company_code='' 是兜底配置，表示所有没有特殊配置的都走此默认值
5. **预报价配置**：version=3 是特殊配置，目前只有上海市配置了预报价
